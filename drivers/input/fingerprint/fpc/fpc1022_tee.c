...
#define FPC1022_POWER_LOW_US 10000
#define FPC1022_POWER_HIGH_US 10000
...
#define FPC1540_CHIP 0x1800
#define FPC1540_CHIP_MASK_SENSOR_TYPE 0xff00
//for FPC1542 and FPC1542SA
#define FPC1542_CHIP_L11 0x1e00
...
static void fpc1022_get_irqNum(struct fpc1022_data *fpc1022)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,fpc1022");
...
static int hw_reset(struct fpc1022_data *fpc1022)
{
	...
	dev_err(dev, "%s, hw_reset reset low !\n", __func__);
	...
	usleep_range(FPC1022_POWER_LOW_US, FPC1022_POWER_LOW_US + 100);
	dev_err(dev, "%s, hw_reset power low !\n", __func__);

#if defined(CONFIG_PRODUCT_XAGA_FP)
	ret = regulator_set_voltage(buck, 1800000, 1800000);
#else
	ret = regulator_set_voltage(buck, 3300000, 3300000);
#endif
	if (ret < 0) {
	...
	if (ret < 0) {
		dev_err(dev, "regulator_set_load fail");
	}


	if (regulator_is_enabled(buck)) {
		dev_err(dev, "%s, fp_vdd_vreg is already enabled!\n", __func__);
	}

	ret = regulator_enable(buck);
	if (ret < 0) {
		dev_err(dev, "%s, regulator_enable fail!!\n", __func__);
	}

	ret = regulator_get_voltage(buck);
	dev_info(dev, "%s, regulator_value %d!!\n", __func__,ret);

	usleep_range(FPC1022_POWER_HIGH_US, FPC1022_POWER_HIGH_US + 100);
	dev_err(dev, "%s, hw_reset power high !\n", __func__);

	pinctrl_select_state(fpc1022->pinctrl, fpc1022->st_rst_h);
	usleep_range(FPC1022_RESET_HIGH1_US, FPC1022_RESET_HIGH1_US + 100);
	dev_err(dev, "%s, hw_reset reset high !\n", __func__);
...
static int fpc1022_platform_probe(struct platform_device *pldev)
{
...
#if defined(CONFIG_PRODUCT_RUBENS_FP)
	buck = regulator_get(NULL, "mt6368_vfp");
	dev_info(dev, "%s, get rubens vfp!!\n", __func__);
#elif defined(CONFIG_PRODUCT_XAGA_FP)
	buck = regulator_get(NULL, "mt6368_vibr");
	dev_info(dev, "%s, get xaga vibr!!\n", __func__);
#else
	buck = regulator_get(NULL, "mt6373_vfp");
	dev_info(dev, "%s, get matisse vfp!!\n", __func__);
#endif
	if (buck == NULL) {
		dev_err(dev, "regulator_get fail");
	...
	if (ret < 0) {
		dev_err(dev, "regulator_set fail");
		dev_err(dev, "current voltage is %d", regulator_get_voltage(buck));
		goto err_pinctrl_get;
	}
...
static int check_hwid(struct spi_device *spi)
{
	...
	do {
		...
		if (((FPC1540_CHIP_MASK_SENSOR_TYPE & hardware_id) == FPC1540_CHIP )||
			((FPC1542_CHIP_MASK_SENSOR_TYPE & hardware_id) == FPC1542_CHIP )||
			((FPC1542_CHIP_MASK_SENSOR_TYPE & hardware_id) == FPC1542SA_CHIP)||
			((FPC1542_CHIP_MASK_SENSOR_TYPE & hardware_id) == FPC1542_CHIP_L11)) {
